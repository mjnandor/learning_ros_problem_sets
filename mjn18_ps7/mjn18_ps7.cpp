...
geometry_msgs::PoseStamped g_block_position;


void poseCallback(const geometry_msgs::PoseStamped& msg_input){
    
    g_block_position = msg_input;
    ROS_ERROR("New Position Received");
...
int main(int argc, char** argv){
    ros::init(argc, argv, "ps7_commander");
    ros::NodeHandle nh;

    //subscribe to block info
    ros::Subscriber my_subscriber = nh.subscribe("/block_pose", 1, poseCallback);
    ...
    while (ros::ok()) {
        ...
        tool_pose.pose.position.z = 0.2; //0.01;          
        ...
        //get block position
        ros::spinOnce();
        tool_pose.pose.position = g_block_position.pose.position;


        //move to approach pose:
        ROS_INFO("moving to approach pose");
        //modify here, get block position from camera
        tool_pose.pose.position.z = 0.1; //0.01;     
        ROS_INFO("requesting plan to descend:");
        xformUtils.printPose(tool_pose);
        rtn_val = cart_motion_commander.plan_cartesian_traj_qprev_to_des_tool_pose(nsteps, arrival_time, tool_pose);
        if (rtn_val == arm_motion_action::arm_interfaceResult::SUCCESS) {
