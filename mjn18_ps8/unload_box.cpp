...
void qc2_callback(const osrf_gear::LogicalCameraImage& camera_msg){
...
int main(int argc, char** argv) {
    ...
    ros::Subscriber qc2_subscriber_object = nh.subscribe("/ariac/quality_control_sensor_1", 1, qc2_callback);
    bool status;    
    int nparts;
    ...
    if (boxInspector.get_bad_part_Q1(current_part)) {
        ...
        int num_parts = orphan_models_wrt_world.size();
    ...
    boxInspector.update_inspection(desired_models_wrt_world,
        ...
        ROS_INFO("num parts seen in box = %d",nparts);
        for (int i=0;i<nparts;i++) {
            ROS_INFO_STREAM("removing part: "<<orphan_models_wrt_world[i]<<endl);
