...
int main(int argc, char** argv)
{
    ...
    double turret_speed = 1; // rad/sec
    double shoulder_speed = -2; // rad/sec
    double amp3 = M_PI/2;
    double amp4 = M_PI;
    double amp5 = 0;
    double wrist_speed = 10; //rad/sec //ALL THE PRETTY THINGS ARE BROKEN
    ...
    for (int ipt =0;ipt<n_traj_pts;ipt++) {
             trajectory_point1.positions[0] += turret_speed*dt;
             trajectory_point1.positions[1] += shoulder_speed*dt;
             trajectory_point1.positions[2] = amp3*sin(omega*t) + M_PI/2;
             trajectory_point1.positions[3] = amp4*sin(omega*t);
             trajectory_point1.positions[4] = amp5*sin(omega*t);
             trajectory_point1.positions[5] += wrist_speed*dt;
